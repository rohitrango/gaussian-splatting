...
class SMPLGaussianModel:
    ...
    def create_from_SMPL(self, smpl_mesh, faces, per_face_color: torch.Tensor, spatial_lr_scale: float):
        ...
        fused_color = RGB2SH(per_face_color.float().cuda())
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0] = fused_color
        features[:, 3:, 1:] = 0.0
        self._faces = torch.arange(N, device="cuda").long()
        ...
        facearea = torch.norm(torch.cross(v2 - v1, v3 - v1), dim=-1) / 2.0  # [Nf]
        scales = torch.log(facearea[:, None].repeat(1, 3) + 1e-10)
        rots = torch.zeros((N, 4), device="cuda")
        rots[:, 0] = 1
        opacities = inverse_sigmoid(0.1 * torch.ones((N, 1), dtype=torch.float, device="cuda"))

        # set parameters
        self._xyz = nn.Parameter(per_face_color.clone().requires_grad_(True))
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # TODO: change this to project along the barycentric coordinates, right now it just randomly scatters
        # wherever, since 'xyz' are actually the barycentric coordinates
        stds = self.get_scaling[selected_pts_mask].repeat(N, 1)*0 + 0.5
...
## Base Gaussian model given in the codebase
class GaussianModel:
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            L = build_scaling_rotation(scaling_modifier * scaling, rotation)
